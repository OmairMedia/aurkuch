...
import com.basitobaid.youtubeapp.apiCalls.GetSubscriptions;
import com.basitobaid.youtubeapp.dialog.ReferralCodeDialog;
import com.basitobaid.youtubeapp.fragment.CategoyFragment;
import com.basitobaid.youtubeapp.fragment.ChannelFragment;
import com.basitobaid.youtubeapp.fragment.ChatActivity;
import com.basitobaid.youtubeapp.fragment.FBvideoFragment;
import com.basitobaid.youtubeapp.fragment.InviteFriendsFragment;
import com.basitobaid.youtubeapp.fragment.ProfileFragment;
import com.basitobaid.youtubeapp.fragment.SettingsFragment;
import com.basitobaid.youtubeapp.fragment.VideosFragment;
...
public class MainActivity extends AppCompatActivity
        implements EasyPermissions.PermissionCallbacks, BottomNavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (from != null && type != null && from.equals("notify")) {
            if (type.equals("Chat")) {
            ...
            if (AppPreferences.getFirstTime()) {
                showReferralDialog();
            } else {
                deductWallet();
            }
        ...
        findViewById(R.id.checkapi_btn).setOnClickListener(v -> {
//                initWebEngine();
...
//        activityLayout.addView(mCallApiButton);
        ...
        mOutputText = findViewById(R.id.text_view);
//        mOutputText.setVerticalScrollBarEnabled(true);
//        mOutputText.setMovementMethod(new ScrollingMovementMethod());
        mOutputText.setText(
                String.format("Click the '%s' button to test the API.", BUTTON_TEXT));
...
//        Toast.makeText(this, mCredential.getSelectedAccountName(), Toast.LENGTH_SHORT).show();


//        new Handler().postDelayed(this::getResultsFromApi,1000);
    }
    ...
    private void startFirstFragment() {
        ...
        getSupportFragmentManager()
                .beginTransaction()
                .add(R.id.fragment_container, CategoyFragment.newInstance())
                .addToBackStack(BASE_FRAGMENT)
                .commit();
    }

    private void showReferralDialog() {
        ReferralCodeDialog dialog = new ReferralCodeDialog(new ReferralCodeDialog.ReferralDialogCallback() {
            @Override
        ...
        dialog.show(getSupportFragmentManager(), "refDialog");
        dialog.setCancelable(false);
    }

    private void checkReferralCode(String code) {
        Call<Response.CommonResponse> call = RestClient.getClient().updateWallet(AppPreferences.getUserToken(), "0", code);
        call.enqueue(new Callback<Response.CommonResponse>() {
            ...
            public void onResponse(@NonNull Call<Response.CommonResponse> call,
                ...
                if (response.isSuccessful()) {
                    DialogUtils.showCongratsDialog(MainActivity.this, "100");
                } else {
                    if (response.body() != null) {
                        DialogUtils.showInfoDialog(MainActivity.this, response.body().message);
                    }
                }
                AppPreferences.setFirstTime(false);
            ...
            public void onFailure(@NonNull Call<Response.CommonResponse> call,
                                  @NonNull Throwable t) {
                DialogUtils.showErrorDialog(MainActivity.this, "error occurred");
            }
        });
    }

    private void deductWallet() {
        Call<Response.CommonResponse> call = RestClient.getClient().deductWallet(AppPreferences.getUserToken(), "0");
...
//                youTubePlayer.loadVideo(videoId, 0);
            ...
            public void onCurrentSecond(@NonNull YouTubePlayer youTubePlayer, float second) {
                ...
                mOutputText.setText(String.valueOf(second));
            }
        });
    }
    ...
    private void getResultsFromApi() {
        if (! isGooglePlayServicesAvailable()) {
            Log.d("TAG", "getResultsFromApi: acquire google play services");
            acquireGooglePlayServices();
        } else if (mCredential.getSelectedAccountName() == null) {
            Log.d("TAG", "getResultsFromApi: choose account");
            chooseAccount();
        } else if (!isDeviceOnline()) {
            mOutputText.setText(getString(R.string.no_connection_string));
        } else {
...
//            }
//            initWebEngine();
            Log.d("TAG", "getResultsFromApi: calling api");
    ...
    private void callNextActivity() {
        ...
        intent.putExtra(Constant.URL, "https://m.youtube.com/user/GoogleDevelopers");
//        intent.putExtra(Constant.URL, "https://www.youtube.com");
...
//                if (commandType.equals(Constant.GET_USER_CHANNEL)) {
//                    getUserChannelInfo();
//                } else {
        ...
        switch(requestCode) {
            case REQUEST_GOOGLE_PLAY_SERVICES:
                if (resultCode != RESULT_OK) {
                    mOutputText.setText(getString(R.string.google_play_services_string));
                } else {
                    getResultsFromApi();
                }
                break;
            case REQUEST_ACCOUNT_PICKER:
                if (resultCode == RESULT_OK && data != null &&
                    ...
                    if (accountName != null) {
                        AppPreferences.setUserAccountName(accountName);
        ...
        Fragment fragment = getSupportFragmentManager().findFragmentById(R.id.fragment_container);
            ...
            switch (id) {
                case R.id.navigation_home:
                    if (!(fragment instanceof CategoyFragment)) {
                        getSupportFragmentManager().popBackStack(BASE_FRAGMENT, FragmentManager.POP_BACK_STACK_INCLUSIVE);
                        getSupportFragmentManager()
                                .beginTransaction()
                                .add(R.id.fragment_container, CategoyFragment.newInstance())
                                .addToBackStack(BASE_FRAGMENT)
                ...
                case R.id.navigation_invite:
                    if (!(fragment instanceof InviteFriendsFragment)) {
                        ...
                        getSupportFragmentManager()
                                .beginTransaction()
                                .add(R.id.fragment_container, InviteFriendsFragment.newInstance())
                                .addToBackStack(BASE_FRAGMENT)
                                .commit();
                    }
                    break;
